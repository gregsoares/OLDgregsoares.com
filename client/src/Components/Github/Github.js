  ...
  import { mockSearchRepo, mockFetchGregsoaresRepo } from "../../api/github";
  // import metalBG from "./greymetal.svg";
  import blueCardBG from './blueCardBG.svg'

  export const Github = () => {
    const [repoData, setRepoData] = useState(null);
    const [gregsoaresRepo, setGregsoaresRepo] = useState(null);
  ...
  function displayResult() {
    // <p data-testid="displayResult">{(JSON.stringify(repoData.items[0]))} </p>
        ...
        <div className="border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal" style={{ "backgroundImage": `url('${blueCardBG}')` }} >
          <div className="mb-8">
            <p className="text-sm text-gray-600 flex items-center">
              <span className="text-teal-300">        Repository Name: {(JSON.stringify(repoData.items[0].name).replace(/"/g, ""))} </span>
            </p>
            <div className="text-teal-100 font-bold text-lg mb-2">{JSON.stringify(repoData.items[0].description).replace(/"/g, "")} </div>
  ...
  return (
    <div data-testid="GithubContainer">
      ...
      {repoData != null ? (displayResult()) : "displayResult() Loading"}
